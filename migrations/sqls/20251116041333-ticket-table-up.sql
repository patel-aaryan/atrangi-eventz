...
CREATE TABLE IF NOT EXISTS tickets (
    ...
    -- User relationship (nullable for guest purchases)
    user_id UUID REFERENCES users(id) ON DELETE SET NULL,
    
    -- Order information (shared across all tickets in the same purchase)
    order_id UUID NOT NULL, -- Groups tickets from the same purchase
    ...
    subscribe_to_newsletter BOOLEAN DEFAULT false, -- Newsletter subscription preference
    agreed_to_terms BOOLEAN DEFAULT false, -- Terms and conditions agreement
...
CREATE INDEX idx_tickets_user_event ON tickets(user_id, event_id) WHERE user_id IS NOT NULL;
...
COMMENT ON COLUMN tickets.subscribe_to_newsletter IS 'Whether buyer opted into newsletter';
COMMENT ON COLUMN tickets.agreed_to_terms IS 'Whether buyer agreed to terms and conditions';
